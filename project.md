...
- **[timeline]** : 2026-08-28 - 줌인 비교 모달 디테일 한계 극복을 위한 하이브리드 해상도(Dual-Resolution) 온디맨드 렌더링 아키텍처 추가
- **[issue]** : 대량 로딩 OOM 방어용 1080px WebP 썸네일 전역 사용 시, 줌인 비교 모달(`CompareModal`)에서 2.5x~8x 고배율 확대 시 얼굴/눈동자 디테일이 픽셀화되어 베스트 컷 선별이 어려운 한계 발생.
- **[Architecture]** : **하이브리드 해상도(Dual-Resolution) 온디맨드 렌더링 파이프라인 도입**
  - **Overview Grid (`GroupGrid`)**: 1,000장 이상 대량 로드 시 브라우저 OOM 방어를 위해 1080px WebP 경량 썸네일 유지.
  - **Detail Zoom (`CompareModal`)**: 활성 모달 검사 대상 사진에 한해 Zero-Egress로 브라우저에 보관 중인 로컬 원본(`File`) 객체를 온디맨드로 일시 로드(`URL.createObjectURL`)하여 **4K/24MP 네이티브 해상도로 렌더링**.
  - **Memory Lifecycle Management**: 모달 종료 및 이미지 전환 시 `URL.revokeObjectURL`을 즉시 호출하여 메모리 누수를 원천 차단.
- **[Performance]** : 브라우저 OOM 방어(1,000장+ 안정 구동)와 줌인 시 4K/24MP 원본급 초고화질 디테일(속눈썹/눈동자 선명도 100%)을 동시에 달성.